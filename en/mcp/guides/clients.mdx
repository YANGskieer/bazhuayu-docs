...
> 除 [Coze](/en/mcp/integrations/coze)、[Dify](/en/mcp/integrations/dify)、[QClaw](/en/mcp/integrations/qclaw)、[企业微信](/en/mcp/integrations/wecom) 外，常见 AI 客户端的八爪鱼 MCP 连接方式。
...
认证方式：**OAuth 2.1**（推荐）或 **API Key**（Header：`x-api-key`）。API Key 创建见 [获取 API Key](/en/mcp/quick-start/api-key)。

## ChatGPT

1. 打开 ChatGPT → **Settings** → **Connectors**（或「连接器」）。
2. 添加 MCP 连接器，URL 填写 `https://mcp.bazhuayu.com` 或 `https://mcp.octoparse.com`。
3. 选择 **OAuth** 完成八爪鱼 / Octoparse 登录，或使用 API Key（若界面支持自定义 Header）。
...
或使用 OAuth 流程（以 Claude Code 当前版本文档为准）。详见 [获取 API Key](/en/mcp/quick-start/api-key)。
...
QClaw 用户也可使用 [QClaw 图形界面配置](/en/mcp/integrations/qclaw)，无需命令行。
...
<CardGroup cols={2}>
  <Card title="工作流程" href="/en/mcp/workflow">
    搜模板 → 运行 → 导出完整示例。
  </Card>
