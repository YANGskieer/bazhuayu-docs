...
八爪鱼 CLI 是面向开发者、数据团队、运维和 AI Agent 场景的命令行工具。CLI 国内 v0.1.23 将网页识别入口统一为 `octopus detect`，并修复、优化了详情页与分页检测能力。
...
<Steps>
  <Step title="安装 CLI">
    使用 `npm install -g bazhuayu-cli@0.1.23` 安装，并运行 `octopus doctor` 检查环境。
  </Step>
  <Step title="登录认证">
    通过 `octopus auth login`、`--oauth`、`--stdin` 或环境变量完成认证。
  </Step>
  <Step title="选择采集路径">
    新 URL 用 `detect --output task.json` 生成任务，已有任务或任务文件用 `run` 执行。
  </Step>
  <Step title="运行与监控">
