...
八爪鱼 CLI 是面向开发者、数据团队、运维和 AI Agent 场景的命令行工具。v0.1.27 强化了 Agent 任务修正、API 列表候选、可见 DOM 字段选择与自定义候选区能力，并增强了 `doctor` 诊断参数。
...
<Steps>
  <Step title="安装 CLI">
    使用 `npm install -g bazhuayu-cli@0.1.27` 安装，并运行 `octopus doctor` 检查环境。
  </Step>
  <Step title="登录认证">
