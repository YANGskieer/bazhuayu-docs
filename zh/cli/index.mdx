...
八爪鱼 CLI 是面向开发者、数据团队、运维和 AI Agent 场景的命令行工具。v0.1.25 重构了 detector 智能识别模块，改进候选区排序与分页识别准确率，并新增 detect 生成任务自动同步到客户端任务列表的能力。
...
<Steps>
  <Step title="安装 CLI">
    使用 `npm install -g bazhuayu-cli@0.1.25` 安装，并运行 `octopus doctor` 检查环境。
  </Step>
  <Step title="登录认证">
