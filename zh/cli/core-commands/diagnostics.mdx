...
`doctor` 会检查 Node.js、CLI 运行时、浏览器依赖、认证/API 连通性和本地运行目录可写性。v0.1.27 起可用 `--output <dir>` 指定运行目录检查位置，用 `--api-base-url <url>` 指定 API 诊断地址。若某项为 `ok: false`，按提示修复后再运行采集任务。
...
| `detect --agent --yes` 仍在旧脚本里 | v0.1.27 不再需要 `--yes`，但仍接受该参数以兼容旧脚本；新脚本请省略 |
| API 列表任务无法同步到云端 | API 列表任务当前 `localOnly: true`，使用 `octopus run <taskId> --task-file task.json` 本地运行 |
| 安装 `@latest` 卡住 | 使用固定版本：`npm install -g bazhuayu-cli@0.1.27` |
| QClaw 升级后 `octopus` 命令找不到 | QClaw 版本号目录变化导致 node 路径失效，更新 PATH 中的 QClaw 版本目录 |
