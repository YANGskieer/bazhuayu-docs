...
> `octopus detect` 是网页检测与任务生成入口。v0.1.28 保持 detect / Agent 接口兼容；v0.1.27 强化的 Agent 任务修正、API 列表候选、可见 DOM 字段选择和自定义候选区能力继续适用。

## v0.1.28 兼容说明

v0.1.28 的主要新增能力集中在任务管理、任务组、模板任务、云定时和数据检查命令，`detect`、Agent 计划、`--run-sample`、API 列表任务和本地 `run` 语法保持兼容。新建任务仍推荐：
