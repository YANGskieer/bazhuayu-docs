...
> `octopus detect` 是网页检测与任务生成入口。v0.1.30 支持使用系统 Chrome / Edge 用户 Profile 检测登录后的页面，并继续兼容既有 Agent 计划、API 列表候选、可见 DOM 字段选择和自定义候选区能力。
...
octopus detect "https://example.com/account" --manual --browser user --browser-id edge --profile "Profile 1" --output task.json
octopus detect "https://example.com/account" --agent --agent-command "node make-plan.mjs" --browser user --browser-id chrome --profile "Default" --goal "提取账户数据" --run-sample 3 --json
```

首次使用前必须通过 [`octopus browser`](/zh/cli/core-commands/browser-management) 安装扩展，并确认 `browser status --json` 的 `readyForUserBrowserRun` 为 `true`。用户模式仅支持 Windows 和 macOS，不支持 `--headless`。日常 detect 会复用已打开的浏览器并创建专用会话窗口；只有 `browser install` 要求浏览器完全关闭。
...
```bash
octopus detect URL --agent --agent-command "node make-plan.mjs" --goal "采集目标" --run-sample 3 --json
octopus detect URL --auto --goal "采集目标" --task-id <taskId> --output task.json
...
API 列表任务会在 JSON 输出中标记 `generatedTask.mode: "api_list"` 与 `localOnly: true`。这类任务可以用 `octopus run <taskId> --task-file task.json` 本地运行，当前不自动同步云端任务。
...
octopus detect --apply-agent-plan plan.json --agent-context context.json --output task-fixed.json --json
```
...
octopus detect URL --manual --browser user --browser-id chrome --profile "Default" --output task.json
```
