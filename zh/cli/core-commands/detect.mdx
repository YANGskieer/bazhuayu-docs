...
> `octopus detect` 是网页检测与任务生成入口。v0.1.26 优化了 `detect --auto` 候选区排序算法，修复了 Windows 上 URL 生成任务名的异常，增大了 Agent 模式截图标注面积，并将 detect 命令底层实现按能力拆分为独立模块。v0.1.25 建立的 detector 重构、任务同步客户端等能力，以及 v0.1.24 建立的 Agent 视觉审查与样品采集流程继续适用。
