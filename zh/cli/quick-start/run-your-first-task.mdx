...
`--agent-command` 是本地 shell 命令，不是自然语言提示。v0.1.27 不再需要 `--yes`；旧脚本传入时仍会被兼容。`--run-sample` 只支持 `--agent` 且必须为正整数。命令返回单个 JSON envelope；任务生成结果与样品采集结果应分别检查，尤其是 `sampleRun.exitCode` 和 `sampleRun.summary`。
