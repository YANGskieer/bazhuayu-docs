---
title: 安装与验证
description: 安装 bazhuayu-cli@0.1.25，并验证 octopus 命令、Node.js 环境与本地采集运行时。
---

> 通过 npm 安装八爪鱼 CLI，并确认 `octopus` 命令、浏览器依赖与本地运行环境可用。

八爪鱼 CLI 以 npm 包形式发布。v0.1.25 重构了 detector 智能识别模块，改进候选区排序与分页识别准确率，并新增 detect 生成任务自动同步到客户端的能力。
...
| `npm install -g ...@latest` 卡住或被终止 | 使用固定版本：`npm install -g bazhuayu-cli@0.1.25` |
