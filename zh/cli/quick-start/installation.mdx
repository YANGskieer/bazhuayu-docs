---
title: 安装与验证
description: 安装 bazhuayu-cli@0.1.22，并验证 octopus 命令、Node.js 环境与本地采集运行时。
---

> 通过 npm 安装八爪鱼 CLI，并确认 `octopus` 命令、浏览器依赖与本地运行环境可用。

八爪鱼 CLI 以 npm 包形式发布。CLI 国内 v0.1.22 新增网页识别建任务、URL 直接采集与 Linux x64 支持。
...
v0.1.22 应能看到 `recognize` 与 `run-url` 相关命令：

```bash
octopus recognize --help
octopus run-url --help
```
...
| `npm install -g ...@latest` 卡住或被终止 | 使用固定版本：`npm install -g bazhuayu-cli@0.1.22` |
