---
title: 安装与验证
description: 安装 bazhuayu-cli@0.1.26，并验证 octopus 命令、Node.js 环境与本地采集运行时。
---

> 通过 npm 安装八爪鱼 CLI，并确认 `octopus` 命令、浏览器依赖与本地运行环境可用。

八爪鱼 CLI 以 npm 包形式发布。v0.1.26 优化了 `detect --auto` 候选区排序算法，修复了 Windows 上 URL 生成任务名的异常，增大了 Agent 模式截图标注面积，并将 detect 命令底层实现按能力拆分为独立模块。v0.1.25 建立的 detector 重构、任务同步客户端等能力继续适用。
...
| `npm install -g ...@latest` 卡住或被终止 | 使用固定版本：`npm install -g bazhuayu-cli@0.1.26` |
