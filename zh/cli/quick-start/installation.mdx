---
title: 安装与验证
description: 安装 bazhuayu-cli@0.1.23，并验证 octopus 命令、Node.js 环境与本地采集运行时。
---

> 通过 npm 安装八爪鱼 CLI，并确认 `octopus` 命令、浏览器依赖与本地运行环境可用。

八爪鱼 CLI 以 npm 包形式发布。v0.1.23 使用 `octopus detect` 作为网页检测与任务生成入口。
...
| `npm install -g ...@latest` 卡住或被终止 | 使用固定版本：`npm install -g bazhuayu-cli@0.1.23` |
