---
title: 命令速查表
description: 八爪鱼 CLI v0.1.27 常用命令快速参考。
---
...
`--agent-command` 必须是可信的本地 shell 命令；自然语言目标使用 `--goal`。`--run-sample` 只支持 `--agent`，且参数必须是正整数。v0.1.27 不再需要 `--yes`；该参数仍为旧脚本兼容保留。v0.1.25 起，detect 生成的普通任务会自动同步到客户端任务列表；API 列表任务当前仅支持本地运行。
